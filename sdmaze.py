...
class Maze:
	...
	def __init__(self, linesInText):
		...
		self.maze=[['_' for row in range(self.columns)] for row in range(self.rows)]# creating a 2D matrix; initialized to '_''
		self.start = None
		self.goal = None
		self.blocks = []
	...
	def aStarSearch(self, type):
		...
		while(len(openList) > 0 and found == False):
			if(type == "Euclidean"):
				openList = (sorted(openList, key = lambda node:node.fEuclidean))	# sorts the openList list at each iteration
			elif(type == "Manhattan"):
